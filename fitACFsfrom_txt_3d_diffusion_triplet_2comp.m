...
if twocomponents==0
    ...
    fitparameterlist=zeros(size(files,1),9);
end
...
for i=1:size(files,1)
    ...
    tcorrsegfinal=ACFdata(5:end,1);
    fcorrsegfinal=ACFdata(5:end,2)-1;
    weightssegfinal=(1:1:length(tcorrsegfinal))/length(tcorrsegfinal)';
    ...
    if triplett==1
        if twocomponents==0
            ...
            if failed==0
                ...
                x0=[N0, 10^-4, S, 0.2, 30*10^-6, alpha];
                lbfit=[0, 20^-5, S, 0, 0.1*10^-6, alpha];
                ubfit=[5000, 0.01, S, 0.9, 100*10^-6, alpha];
            end
...
%             close(fitshow)
            ...
            N0=1/(mean(fcorrsegfinal(1:3)));
            x0=[N0,ffree,3*10^-4,10^-3, S, 0.1, 4*10^-6, alpha,alpha];
            
            lbfit=[0,0,5*10^-5,1*10^-3, S, 0, 1*10^-7, alpha,alpha];
    
            ubfit=[5000,1,5*10^-3,10^-1, S, 0.9, 10^-4, alpha, alpha];
...
% %             fixed=[false false false false false false false false false];
            ...
            fitparameterlist(i,:)=[Nsegfinal ffreefit taud1segfinal taud2segfinal Tfractionsegfinal Ttausegfinal alpha1segfinal alpha2segfinal Sfitsegfinal];
        end
    else
...
path2=uigetdir;
filename=sprintf('/fog.txt'); %Does that also work for windows, or do we need \ there?
...
if twocomponents==0
    ...
    fprintf(fid1,'sample\t N\t f free\t tau free\t tau bound\t T\t tauT\t alpha free\t alpha bound\t S\n');
    for i=1:size(files,1)
        fprintf(fid1,files(i).name);
